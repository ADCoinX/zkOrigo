...
def generate_pain001_from_result(result: dict) -> str:
    ...
    ET.SubElement(grp, "CreDtTm").text = result.get(
        "timestamp", datetime.datetime.utcnow().isoformat() + "Z"
    )
    ET.SubElement(grp, "NbOfTxs").text = "1"

    # Ambil balance (convert ke float jika boleh)
    balance = result.get("balance", 0)
    try:
        balance_val = float(balance) if balance not in (None, "", "0") else 0.0
    except Exception:
        balance_val = 0.0

    ET.SubElement(grp, "CtrlSum").text = f"{balance_val:.2f}"
    ...
    ET.SubElement(
        rmt, "Ustrd"
    ).text = f"AI Risk Score: {result.get('risk_score')} - {', '.join(result.get('ai_reason', []))}"
